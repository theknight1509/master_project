...
class Extract(object):
    def __init__(self, dir_name):
        ...
        self.data_index_filename = dir_string_func(dir_name) + "data_indeces.txt"
    ...
    def get_data_index(self, string):
        ...
        for content_row in loa_content_rows:
            ...
            current_array_name = int(current_row[1])
    ...
    def handle_all_data(self, extract_func, extract_filename):
        ...
        for data_filename in get_numpy_filenames(): #loop over numpy-files
            ...
            loa_extracted_arrays.append(extraced_arr) #store extracted array
        extacted_data = np.stack(loa_extracted_arrays) #stack into matrix of extracted arrays
        extracted_filename = self.dir_string_func(self.save_dir) + "extract_" + extract_filename + ".npy"
        np.save(extract_filename, extracted_data) #save extracted data
        return
    ...
    def get_time_array(self):
        ...
        time_array = single_matrix[index_time,:]
    ...
    def setup_extract_reos(self):
        ...
        for key in loa_keys:
            extract_func = lambda data: extract_single_array(data=data, index_array=doa_array_index[key])
            self.handle_all_data(extract_func=extract_func,
                                 extract_filename=doa_filename[key])
        #extract re187/os-187 ism-array
        extract_func = lambda data: extract_ratio_array(data=data,
...
def Reduce(Extract):
    def __init__(self, dir_name):
        Extract.__init__(self, dir_name)
    ...
    def get_timeevol(self, filename):
        ...
        maximum = np.amax(numpy_matrix, axis=axis)
        minimum = np.amin(numpy_matrix, axis=axis)
    ...
    def get_hist(self, filename, loa_timepoints):
        ...
        for timepoint in loa_timepoints:
            ...
            doa_arrays["t=%1.2fGyr"%timepoint/1.0e+9] = numpy_array
    ...
    def save_pandas(self, save_filename, dictionary):
        ...
        pandaframe.to_csv(self.dir_string_func(self.save_dir) + "reduce_" + save_filename + ".csv")
    ...
    def setup_reduce_reos(self, loa_timepoints=[9.5e+9, 14e+9]):
        ...
        for extracted_filename in loa_extracted_filenames:
            ...
            save_filename = save_filename[len("extracted_"):]
            ...
            hist_dict = self.get_hist(extracted_filename)

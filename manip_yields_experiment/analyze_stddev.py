...
folder = Foldermap()
default_filename_postamble = "deafult.npy" #Final string of default-datafile-name
index_filename = "data_indeces.npy"
...
for content in loa_contents: #loop over all strings in cwd
    ...
    elif (content[-len(default_filename_postamble):] == default_filename_postamble):
        #Found default-file (fudgefactor=1) get generic data-file name and default-file name
        default_filename = content
        data_filename = content[:-len(default_filename_postamble)]

#find folder name
print "Current available folders: ", loa_folders
input_datafolder = raw_input("What folder would you like to choose?")
if not (input_datafolder in loa_folders):
    print "Folder: %s not available"%(input_datafolder)
    sys.exit("Exiting!")
else:
    #print "Folder: %s found"%(input_datafolder)
    None
...
num_timepoints = default_shape[0,:]
...
data_matrix_arrays = np.zeros#(num_experiments, num_timepoints) #TODO #collection of arrays wanted
...
for i in range(num_experiments):
    ...
    data_matrix_arrays[i,:] = data_matrix[array_index,:] #TODO #store wanted array from one simulation

#get median, mean, and sigma from data_matrix
data_mean = np.mean(data_matrix_array, axis=0) #TODO
data_median = np.median(data_matrix_array, axis=0) #TODO
data_sigma = np.std(data_matrix_array, axis=0) #TODO

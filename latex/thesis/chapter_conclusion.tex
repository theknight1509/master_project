...
When \betadecay is applied to the data (see figures \comment{ref to figures with betadecay \ref{fig:MCExperiment-delmax}}) the evolution of \os{187} changes drastically, as does the cosmic clock fraction. The cosmic clock fraction $f_{187}\simeq$\comment{insert estimates of mean and deviation from plot} at $t_{f,sos}$ then appears to be comparable to the observed quantities in appendix comment{ref section with luck cosmochronology calculations \ref{sec:calc-cosmo-chronology}?}, $f_{187}=$\comment{insert value and uncertainty}.

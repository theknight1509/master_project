...
By finding a model of \omegamodel\ that most accurately reproduce the mean-values of \eris\footnote{Since \omegamodel\ is a one-zone model without spatial dimensions, it can only approximate the mean of \eris. Not the spatial resolution.}, a Milky Way-proxy (that resolves \re{187} and \os{187} isotopes)x is found.
...
\begin{table}[h]
  ...
  \caption[\omegamodel-parameters directly adopted from \eris]{
    ...
    The yield-table for binary neutron star mergers (from \mycite{arnould07}) was chosen because it was the only r-process yield-table (out of two available) with the presence of \re{187}.
...
\subsection{Mass}
%method
In \omegamodel a galaxy's mass consist of gas and stars (not counting dark matter from section \ref{sec:cosmology}), and in a one-zone model there is gas and stars inside the box and gas outside the box (see figure \ref{tikz:galaxy-iobox}).
The total baryonic mass inside \eris\ (only at redshift z=0 and z=1) is taken from \mycite{guedes11e} and can be found in table \ref{tab:guedes11-baryonic-mass}.

Supernova feedback will drive an outflow from the galaxy into the surrounding medium (\mycite{fielding16, veilleux05}]). Adding outflow proportional to the supernova rate adds some realisim to the model, and might reproduce some of the spectroscopic features.
...
\begin{table}[h]
  \begin{tabular}{|c|c|l|}
    \hline Parameter & \omegamodel-argument & Value taken from \eris \\
    \hline
    Yield-table AGB\&Massive stars & \texttt{table} & \begin{tabular}{l}\verb|agb_and_massive_stars_nugrid|\\\verb|_MESAonly_fryer12delay_wind_preexp.txt|\end{tabular}\\
    Yield-table type 1A SN & \texttt{sn1a\_table} & \verb|sn1a_ivo13_stable_z.txt| \\
    \begin{tabular}{c} Shape of type 1a SN \\ delay-time dist.\end{tabular} & \texttt{sn1a\_rate} & \texttt{exp} \\
    \begin{tabular}{c} Characteristic timescale of \\ type 1a SN delay-time dist.\end{tabular} & \texttt{exp\_dtd} & \texttt{5.5e+6} [yrs] \\
    \begin{tabular}{c} Number of type 1a SN \\ per stellar mass formed \end{tabular} & \texttt{nb\_1a\_per\_m} & 1e-3 \\
    \hline
  \end{tabular}
  \caption[\omegamodel-parameters from fitting stars and supernovae to \eris]{
    \label{tab:fitting-parameters-sn1a}
    \omegamodel-parameters after fitting yields and type 1a supernovae.
  }
\end{table}
\FloatBarrier
...
\begin{figure}
  ...
  \begin{subfigure}{\subfigwidth}
    \includegraphics[width=\textwidth]{results/resolution_analysis/resolution_difference_euro_logged.png}
    \caption[\todo]{
  ...
  \hfill
  \begin{subfigure}{\subfigwidth}
    \includegraphics[width=\textwidth]{results/resolution_analysis/resolution_difference_oxy_logged.png}
    \caption[\todo]{
  ...
  \hfill
  \begin{subfigure}{\subfigwidth}
    \includegraphics[width=\textwidth]{results/resolution_analysis/resolution_difference_iron_logged.png}
    \caption[\todo]{
...
For varying different parameters simultaneously a similar method was used as in section \comment{results yields \ref{sec:methods-yields}?}, where a ``fudge-factor'' was applied to the \fiduccialomega\ parameter-values. The ``fudge-factor'' was randomly sampled from a gaussian distribution with mean 1.0 and standard deviation equal to the relative uncertainty $\sigma$, from table \comment{ref table rncp arnould uncertianty \ref{tab:arnould-rncp-uncertainty}?}.
